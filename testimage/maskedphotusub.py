...
global model
model = YOLO('/home/afr/ros2_ws/src/testimage/testimage/best.pt')
...
class ImageSub(Node):
    def __init__(self):
        ...
        self.mask_pub = self.create_publisher(Image, 'roi_mask', 10)
    ...
    def timer_callback(self, data):
        ...
        mask = np.zeros((600, 960), dtype=np.uint8) #put height and width pixels accordingly yo
    ...
    # Draw white rectangle in mask for each bounding box (obstacle area = ROI)
                cv2.rectangle(mask, (x1-20, y1-20), (x2+20, y2+20), 255, thickness=-1)  # filled rectangle
        ...
        mask_msg = self.br.cv2_to_imgmsg(masked_image, encoding="bgr8")
        mask_msg.header.stamp = self.get_clock().now().to_msg()
        mask_msg.header.frame_id = "camera_frame"  # Adjust if needed
